...
import 'package:jss_flutter/localization/language/language_en.dart';
...
import 'package:jss_flutter/networking/ApiBaseHelper.dart';

import '../models/Response/ResponseError.dart';



abstract class ResponseBaseLocationPresenter {
  void onResponseSuccess(ResponseBaseLocation responseDto);
  void onError(String errorTxt);
}

class PresenterBaseLocation {
  ResponseBaseLocationPresenter _view;

  PresenterBaseLocation(this._view);

  ApiBaseHelper _helper = ApiBaseHelper();

  Future<dynamic> getBaseLocation(String url, BuildContext context,Map jsObject,) async {
    try {
      ...
      if (response != null) {
        if (!response.containsKey("errors")) {
          ResponseBaseLocation responseDto = ResponseBaseLocation.fromJson(response);
